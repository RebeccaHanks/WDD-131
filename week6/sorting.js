...
console.log("search", searchList(simpleList, "an").sort(compareFn));
  ...
  // function sorting comparing. returning -1 means that it will switch the values. toLowerCase makes sure that they are all converted to lower case before comparing.
  // to sort the opposite way then switch the < > signs between a and b
  ...
  // if I wanted to sort by distance (hikes) in my object, this works because the string starts with a number.
...
// includes looks at the list to see if it includes the substring. T/F
function searchList(list, query) {
    function searchCallback(item) { // do item.description to filter through the descriptions
        return item.toLowerCase().includes(query.toLowerCase());
    }
    //need to make sure that function that I am calling is comparing T/F
